...
else if (text == '.crepeasty'){
const truth =[
        'sebutkan apa kesukaan mu?',
        'percapainya lu di tahun ini ?',
        'gimana mencintai tetapi dia memilih orang lain?',
        'cintailah pacarmu dan jangan lah lupa ibadah!',
        'jika mencintai seorang dari hati tapi dia mencintai orang mending sadar diri ajah',                                                                                   ]
const ttrth = truth[Math.floor(Math.random() * (truth.length))]
conn.sendMessage(id, ttrth, MessageType.text, { quoted: m })
}
else if (text == '.truth'){
const truth =[
...
module.exports = HandleMsg = async (aruga, message) => {
	...
	case 'antilink':		
        ...
        case 'unban':
            ...
            aruga.reply(from, 'Unbanned User!', message.id)
            break
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
